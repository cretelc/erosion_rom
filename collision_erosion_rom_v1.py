...
def accel_radius_erosion_model(dt, hyperparams, thruster, grids, discharge_chamber, facility, operating_conditions):
    ...
    #Gamma_CEX        = Pcex_b*cex_generation_rate(jb, n0_mm3, sigma_p, Vbeam)     # -> ions/s, Gamma^C_CEX 
...
def simple_erosion_model():
    ...
    Pcex_p   = 0.004    # cex ion impingement probabilityin the plume
    Pcex_b   = 1        # cex ion impingement probability between the grids
    ...
    for t in tsteps[1:]:
        # Fill model inputs 
        hyper = (j_bar, I_bar, y, Pcex_p, n_ds, phi_dis)
        thstr = (Rth,  diverg)
