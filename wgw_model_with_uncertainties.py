...
# Input Parameters from Table 1 
    ...
    for i, lg in enumerate(lg_vec):
        ...
        nominal_Va_ebs = array([va.n for va in Va_ebs])
        sigma_Va_ebs = array([va.s for va in Va_ebs])
        high_va = nominal_Va_ebs + sigma_Va_ebs
        low_va = nominal_Va_ebs - sigma_Va_ebs
        print(abs(high_va))
        #print(low_va)
        #print(Jb)
        # Plots
        l = f"lg = {lg*1000:0.2f} mm"
        axs.plot(Jb*1000, abs(nominal_Va_ebs), color='k', label = l)
        axs.fill_between(Jb*1000, abs(high_va), abs(low_va), color='b', alpha=0.2)
        #axs.plot(wgw_dic[run][0],wgw_dic[run][1], color=color[i], linewidth=0, marker='*', label=f'wgw_data {run}')
